...
func New(cfg *Config) (*Foreman, error) {
	...
	f.reviewer = agents.NewReviewer(cfg.Repo.Path, agents.ReviewerConfig{
		...
		UseCodeRabbit: cfg.Review.Tools.CodeRabbit,
...
func (f *Foreman) completeFeature(feature *Feature) {
	...
	// Try to create a PR
	prURL := ""
	prBody := f.buildPRBody(feature)
	pr, err := f.repo.CreatePullRequest(feature.Branch, feature.Name, prBody)
	if err != nil {
		log.Printf("Failed to create PR for feature %s: %v", feature.ID, err)
		// Try to get existing PR URL
		if url, urlErr := f.repo.GetPullRequestURL(feature.Branch); urlErr == nil {
			prURL = url
		}
	} else {
		prURL = pr.URL
...
func (f *Foreman) buildPRBody(feature *Feature) string {
	body := "## Summary\n\n"
	body += feature.Description + "\n\n"

	if feature.Spec != nil {
		body += "## Specification\n\n"
		body += feature.Spec.Summary() + "\n\n"
	}

	if feature.Plan != nil {
		body += "## Implementation Plan\n\n"
		body += feature.Plan.Summary() + "\n\n"
	}
